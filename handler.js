...
export async function participantsUpdate({ id, participants, action }) {
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = 'https://telegra.ph/file/520a1e4307ad8f6017771.png'
                    try {
 ...
 conn.sendButtonDoc(id, 'Group massege', text, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.intro' : 'Aʟᴅɪ X Aɪsʏᴀʜ', fkontak, { contextInfo: { externalAdReply :{ 
                mediaType: 1, 
                title: this.getName(user), 
                thumbnail: await(await fetch(pp)).buffer(), 
                sourceUrl: 'https://chat.whatsapp.com/CGSQhKuRroGGGaTyIGDWIg',
